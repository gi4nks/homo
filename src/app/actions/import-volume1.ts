...
/**
 * IMPORT VOLUME 1
 * Systematically creates the entire Book structure from provided JSON.
 */
export async function importVolume1() {
  const jsonData = {
  ...
  try {
    ...
    for (const char of jsonData.CharactersList) {
      ...
      // Store partial mapping for easier matching (e.g. "Gathor 'Gunner' Leapmind" -> "Gunner")
    ...
    console.log(`Created ${jsonData.CharactersList.length} characters.`);
